  ...
  implements PickableMesh
{
  ...
  /**
   * WebGPU tile material: Lambert (so the sun light and its shadow map shade
   * the imagery) with a TSL node graph that
   *  - over-blends up to MAX_WEBGPU_TILE_SLOTS raster textures (multi-layer),
   *    each with its own UV transform, tint and opacity;
   ...
   * The GLSL injection pipeline (atlas sampling, composite baking) does not
   * exist on this backend; slot state is fed from setupTextures() via
   * userData.webgpuSlots / userData.webgpuHillshade.
   */
  initWebGPUMaterial(globe: Globe, uniforms: CommonUniforms) {
    ...
    const m = new webgpu.MeshLambertNodeMaterial({
      stencilWrite: false,
      color: globe.color,
    });
    ...
    for (let i = 0; i < MAX_WEBGPU_TILE_SLOTS; i++) {
      slots.push({
        has: uniform(0),
    ...
    for (let i = 0; i < MAX_WEBGPU_TILE_SLOTS; i++) {
      const slot = slots[i];
      const sample = slot.node.sample(uvNode.mul(slot.scale).add(slot.offset));
...
${generateTileCommonInjection(maxTextures)}
  ...
  private ensureCorrectMaterialType(
    ...
    // The WebGPU node-material path never flips types.
  ...
  setupTextures(
    ...
    if (mAny.isMeshLambertNodeMaterial === true && m.userData.webgpuSlots) {
      const slots = m.userData.webgpuSlots as {
        has: { value: number };
        node: { value: Texture };
      ...
      // mat.* is authoritative on the full path; rebindTextures passes only
      // hillshade flags, so fall back to the persisted userData state.
      const shows: (number | boolean)[] =
        (mat.shows as (number | boolean)[] | undefined) ??
        m.userData.shows?.value ??
        [];
      const udColors: (Color | number)[] =
        (mat.colors as (Color | number)[] | undefined) ??
        m.userData.colors?.value ??
        [];
      const opacities: number[] =
        (mat.opacities as number[] | undefined) ??
        m.userData.opacities?.value ??
        [];
      const isHillshades: (number | boolean)[] =
        (mat.isHillshades as (number | boolean)[] | undefined) ??
        m.userData.isHillshades?.value ??
        [];
