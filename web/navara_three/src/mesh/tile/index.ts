...
import { createReplacer, getWebGPU } from "../../utils";
import {
  type TextureSlot,
...
type WebgpuSlotNodes = {
  ...
  opacity: any; // UniformNode<number>
  // 1 when the slot samples a compositor-baked render target: WebGPU RT
  // texel row 0 holds NDC y=+1 (WebGL holds y=−1), so the baked content is
  // V-mirrored versus the GL convention the `uv·scale+offset` formula was
  // tuned on, and the sample must read v′ = 1−v. Regular image textures are
  // uploaded pre-flipped by the backend and need no compensation.
  flipV: any; // UniformNode<number>
};
  ...
  implements PickableMesh
{
  ...
  constructor(ctx: EventContext, mesh: MeshAdded) {
    ...
    this.vectorDrape = new VectorDrapeResolver({
      ...
      syncWebGPUSlotNodes: () => this._syncWebgpuSlotNodes(),
    });
  ...
  private async createMesh(
    ...
    if (transform) {
      ...
      // Tile-local geodetic up for skirt shading (see initWebGPUMaterial).
      // The tile's world position is ECEF, so the radial direction at the
      // tile center is the up vector in world space; bring it back into the
      // tile's local frame with the inverse rotation.
      const skirtUp = this.material.userData.uSkirtUp;
      if (skirtUp) {
        ...
        const upWorld = geometry.boundingSphere.center
          .clone()
          .multiply(new Vector3(transform.sx, transform.sy, transform.sz))
          .applyQuaternion(q)
          .add(new Vector3(transform.tx, transform.ty, transform.tz))
          .normalize();
        skirtUp.value.copy(
          upWorld.applyQuaternion(q.clone().invert()).normalize(),
        );
      }
    }
  ...
  createSkirtMesh(
    ...
    if (hasSkirt && skirtPosition && skirtIndices) {
      ...
      // Skirt flag: 0 for terrain vertices, 1 for skirt vertices. The WebGPU
      // node material reads it to shade skirt walls with the tile's up
      // normal — skirt walls are vertical, so their geometric normal is
      // horizontal and would shade dark under a high sun, drawing a 1px
      // line at LOD seams.
      const skirtFlag = new Float32Array(combinedPosition.length / 3);
      skirtFlag.fill(1, position.length / 3);
      const skirtFlagAttribute = new BufferAttribute(skirtFlag, 1);
      geometry.setAttribute("aSkirt", skirtFlagAttribute);
      terrainGeometry.setAttribute("aSkirt", skirtFlagAttribute);
  ...
  /**
   * WebGPU tile material: a TSL node graph that
   *  - over-blends up to `maxTextures` slot textures (multi-layer raster plus
   *    the texturized-vector region, whose slots sample the compositor's
   *    baked drape render targets), each with its own UV transform, tint and
   *    opacity;
   *  - applies the precomputed hillshade normal map (generated by
   *    HillshadeNormalMapGenerator from DEM data) as a tangent-space normal
   *    map, so terrain relief is shaded by the sun.
   *  - pick pass (uPickable): zeroes the display diffuse and emits the
   *    over-blend of the texturized-vector slots only (their baked RTs carry
   *    pick ids) through the emissive term, matching the classic tileShader
   *    pick semantics — raster-only pixels read batchId=0.
   * The base class mirrors the classic selection (shouldUseNormal): Lambert
   * when normal-based shading is needed (sun/hillshade), Basic (unlit)
   * otherwise — classic tiles are MeshBasicMaterial in that case, so a lit
   * WebGPU material would not match their brightness.
   * The GLSL injection pipeline (atlas sampling, MRT composite baking) does
   * not exist on this backend; slot state is fed from setupTextures() and the
   * drape resolvers via userData.webgpuSlots / userData.webgpuHillshade.
   */
  initWebGPUMaterial(
    globe: Globe,
    ...
    const m = new (
      useNormal ? webgpu.MeshLambertNodeMaterial : webgpu.MeshBasicNodeMaterial
    )({
      stencilWrite: false,
      color: globe.color,
    });
    // TSL uniform (not a plain {value} holder): the pick graph below reads it
    // and TileMesh.onBeforePicking/onAfterPicking flip it via `.value`.
    const uPickable = uniform(0);
    m.userData.uPickable = uPickable;
    m.userData.uTime = uniforms.time;
    // setUniforms()/planComposite read this shape.
    m.userData.defines = { USE_HILLSHADE: 0 };
    m.userData.isHillshades = { value: [] };

    // One slot per material texture slot (raster region + texturized-vector
    // region). 10 slots + hillshade + the sun shadow map stay within the
    ...
    m.userData.webgpuSlots = slots;
    m.userData.webgpuHillshade = hillshade;
    m.userData.tileVer = "v3";

    const uvNode = uv();

    // Multi-layer over-blend: later slots composite over earlier ones,
    // weighted by texture alpha * layer opacity * slot-active flag.
    // colorNode replaces materialColor in NodeMaterial, so fold it in as base.
    let color: any = materialColor.rgb;
    // Pick-pass output (classic tileShader.ts semantics): only texturized-
    // vector slots carry a pickable entity — their per-layer RTs are re-baked
    // with pick ids while uPickable is set — so the pick color over-blends
    // just that region from black, without the display tint (matches the
    // classic attr.g mask + straight-through nvr_pickColor). Raster slots
    // contribute nothing, so non-vector terrain reads batchId=0 instead of
    // leaking imagery into the pick buffer.
    let pick: any = vec3(0, 0, 0);
    for (let i = 0; i < this.maxTextures; i++) {
      const slot = slots[i];
      const slotUv = uvNode.mul(slot.scale).add(slot.offset);
      const sample = slot.node.sample(
        vec2(slotUv.x, mix(slotUv.y, slotUv.y.oneMinus(), slot.flipV)),
      );
      const srcA = sample.a.mul(slot.opacity).mul(slot.has);
      color = mix(color, sample.rgb.mul(slot.tint) as any, srcA as any);
      if (i >= this.texturizedSceneIndexFrom) {
        pick = mix(pick, sample.rgb, srcA as any);
      }
    }
    m.colorNode = vec4(color.mul(uPickable.oneMinus()), 1.0);
    // The pick color goes through the emissive term so it survives the
    // light-less pickScene (Lambert tiles would shade black there) — same
    // idiom as the model/polyline/polygon WebGPU pick materials.
    // MeshBasicNodeMaterial has no typed emissiveNode (nor `emissive`), but
    // the node graph honors the override at runtime.
    (m as any).emissiveNode = pick.mul(uPickable);

    // Hillshade: the slot flagged isHillshade carries a precomputed normal map
    // (normals in the tile's uv/tangent frame, [-1,1] packed to [0,1]).
    // normalMap() perturbs the derivative-based TBN frame and returns a
    // view-space normal, matching NodeMaterial's normalNode convention.
    const hsSample = hillshade.node.sample(
      uvNode.mul(hillshade.scale).add(hillshade.offset),
    );
    const hsNormal = normalMap(
      hsSample,
      vec2(hillshade.exaggeration, hillshade.exaggeration),
    );
    m.normalNode = mix(
      normalView as any,
      hsNormal as any,
      hillshade.has as any,
    );

    // Skirt walls are vertical: under a high sun their fragments shade dark
    // and draw a 1px line along LOD seams. Shade skirt fragments with the
    // tile-local geodetic up normal instead, blended by the aSkirt vertex
    // flag. The uniform value is set in createMesh once the tile transform
    // is known. The flag must go through an explicit varying() — a bare
    // attribute() in fragment position silently reads 0 here (same pattern
    // as pickableMeshWrapper's batchId).
    const skirtFlag =
      this.geometry.getAttribute("aSkirt") != null
        ? varying(attribute("aSkirt"), "nvr_aSkirt")
        : null;
    if (skirtFlag != null) {
      const skirtUp = uniform(new Vector3(0, 0, 1));
      m.userData.uSkirtUp = skirtUp;
      m.normalNode = mix(
        m.normalNode as any,
        transformNormalToView(skirtUp) as any,
        skirtFlag as any,
      );
    }
  ...
  private initMaterial(
    ...
    // WebGPU backend: the GLSL injection pipeline (atlas sampling via
    // onBeforeCompile, composite atlas baking) does not run — ShaderMaterial
    // GLSL cannot compile there. Tiles use a TSL node material instead.
    if (this.isWebGPUBackend()) {
      // Escape hatch for A/B regression checks: Options.webgpuDebug.tileBasic
      // forces the plain unlit single-texture material.
      const forceBasic =
        this.ctx.viewContext?.webgpuDebugFlags?.tileBasic ?? false;
      if (!forceBasic) {
        return this.initWebGPUMaterial(
          globe,
          uniforms,
          this.shouldUseNormal(_mat, globe),
        ) as unknown as TileMaterial;
      }
      // tileBasic: unlit imagery via the first raster texture
      // (`setupTextures` assigns `.map`).
      const m = new (getWebGPU().webgpu.MeshBasicNodeMaterial)({
        stencilWrite: false,
        color: globe.color,
      });
      m.userData.uPickable = { value: 0 };
      m.userData.uTime = uniforms.time;
      m.userData.defines = { USE_HILLSHADE: 0 };
      m.userData.isHillshades = { value: [] };
      return m as unknown as TileMaterial;
    }
...
${generateTileCommonInjection(maxTextures)}
  ...
  private ensureCorrectMaterialType(
    ...
    // (WebGPU: initMaterial rebuilds the node material with the matching
    // Basic/Lambert base class, so flipping is safe there too.)
    const needsLambert = this.shouldUseNormal(mat, globe);
    const isLambert = this.isWebGPUBackend()
      ? (this.material as { isMeshLambertNodeMaterial?: boolean })
  ...
  setupTextures(
    ...
    for (let i = 0; i < textureFragmentsLen; i++) {
      ...
      // A texture whose ImageBitmap was closed after upload (tile cache
      // eviction racing a still-bound slot) reports 0x0. Re-uploading it is
      // impossible: mutating colorSpace/filters with needsUpdate would, on the
      // WebGPU backend, destroy the live GPU texture and recreate it from the
      // dead bitmap (WebGL keeps the previous texels on a failed texImage2D).
      // Skip all mutation so the previously uploaded content keeps sampling.
      const tImage = t.image as { width?: number; height?: number } | undefined;
      if (tImage && (tImage.width === 0 || tImage.height === 0)) {
        textures[i] = t;
        continue;
      }
    ...
    // WebGPU backend: bind textures directly on the node material (compositor
    // atlases are not produced on this backend). Keyed on the material itself,
    // not the backend probe, so the branch matches whatever initMaterial built.
    ...
    if (
      (mAny.isMeshLambertNodeMaterial === true ||
        mAny.isMeshBasicNodeMaterial === true) &&
      m.userData.webgpuSlots
    ) {
      // Same resolver re-derivation as the classic tail below: the raster
      // call rebinds baked-slot render targets (Geographic terrain; a no-op
      // on WebMercator), the vector call parks the texturized region on the
    ...
    if (mAny.isMeshBasicNodeMaterial === true) {
      const first = textures.find((t: Texture | null) => t !== null) ?? null;
      if (mAny.map !== first) {
        mAny.map = first;
        mAny.needsUpdate = true;
      }
      return;
    }
  ...
  /**
   * WebGPU tile material: push the per-slot userData arrays (the single source
   * of truth, same as buildCompositeLayers reads) into the node material's
   * slot uniform nodes. Runs from setupTextures after the texture arrays were
   * rebuilt, and from the drape resolvers' bindSlots after they rewrote their
   * region post-bake. No-op unless the material carries WebGPU slot nodes
   * (both the Basic and Lambert node materials built by initWebGPUMaterial).
   */
  private _syncWebgpuSlotNodes(): void {
    const m = this.material;
    if (!m?.userData?.webgpuSlots) return;
    const slots = m.userData.webgpuSlots as WebgpuSlotNodes[];
    const hs = m.userData.webgpuHillshade as {
      has: { value: number };
      node: { value: Texture };
    ...
    const textures: (Texture | null)[] = m.userData.textures?.value ?? [];
    const shows: (number | boolean)[] = m.userData.shows?.value ?? [];
    const udColors: (Color | number)[] = m.userData.colors?.value ?? [];
    const opacities: number[] = m.userData.opacities?.value ?? [];
    const isHillshades: (number | boolean)[] =
      m.userData.isHillshades?.value ?? [];
    ...
    for (let i = 0; i < slots.length; i++) {
      ...
      const isHs = !!isHillshades[i];
      const shown = !!shows[i] && t !== null;

      if (isHs && shown && !hsBound) {
        hsBound = true;
        hs.node.value = t;
        hs.has.value = 1;
        if (uvOffsets[i]) hs.offset.value.copy(uvOffsets[i]);
        if (uvScales[i]) hs.scale.value.copy(uvScales[i]);
        hs.exaggeration.value = m.userData.hillshadeExaggeration?.value ?? 1;
        continue;
      }
      ...
      // everything else (image textures, the shared empty placeholder) is
      // already in the GL convention the sample formula assumes.
      slot.flipV.value =
        active &&
        (t as { isRenderTargetTexture?: boolean }).isRenderTargetTexture ===
          ...
          ? 1
          : 0;
      if (uvOffsets[i]) slot.offset.value.copy(uvOffsets[i]);
      else slot.offset.value.set(0, 0);
      if (uvScales[i]) slot.scale.value.copy(uvScales[i]);
      else slot.scale.value.set(1, 1);
      const c = udColors[i];
      if (c !== undefined) slot.tint.value.set(c as Color | number);
      else slot.tint.value.set(1, 1, 1);
      slot.opacity.value = opacities[i] ?? 1;
    }
    if (!hsBound) hs.has.value = 0;
  }
